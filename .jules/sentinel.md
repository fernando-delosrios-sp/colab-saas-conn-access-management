...
## 2025-02-28 - [Velocity Template Sandbox Escape Mitigation via Dynamic Index Evasion]

**Vulnerability:** The existing `isUnsafeVelocityAST` validation for Velocity templates in `src/utils/velocity.ts` only blocked access to a few properties when referenced via direct string/identifier literals. Attackers could evade AST-based literal checks by hiding banned properties in variables, or concatenating them at runtime via dynamic index accessors (e.g., `[$var]` or `["con" + "structor"]`), leading to Sandbox Escape / Prototype Pollution.
**Learning:** Checking only explicitly defined property names or hardcoded indices is insufficient for securing template engines against SSTI. If dynamic/non-literal indices are allowed (e.g., non-string, non-integer nodes), attackers can resolve banned properties at runtime by manipulating variables or expressions.
**Prevention:** In addition to adding `process`, `require`, and `global` to the ban list, explicitly block any non-literal (e.g., non-string/non-integer) index lookups in `isUnsafeVelocityAST`. If a dynamic index node evaluates at runtime, we proactively fail the AST validation before compilation.
