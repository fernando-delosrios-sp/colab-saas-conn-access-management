...
## 2025-02-28 - [Sandbox Escape via Dynamic Velocity Index Evaluation]

**Vulnerability:** The existing `isUnsafeVelocityAST` check successfully blocked direct and literal index string accesses (e.g. `obj.constructor` or `obj["constructor"]`), but failed to evaluate dynamic or variable-driven index property lookups (e.g. `obj[$myvar]`, `obj["con" + "structor"]`). Since `velocityjs` doesn't enforce strict execution sandboxing during evaluation, attackers could bypass AST checks by constructing malicious strings during runtime via variables or math expressions. Furthermore, specific unsafe global identifiers (`process`, `require`, `global`) were not restricted at the AST root.
**Learning:** Checking literal strings in AST nodes is insufficient if the template engine evaluates dynamic index accesses at runtime. Attackers can conceal dangerous property references inside variables or concatenated expressions.
**Prevention:** In engines like `velocityjs`, AST validation must strictly forbid non-literal (i.e., non-string, non-integer) expressions inside index access nodes to prevent dynamic property evaluation. Additionally, global execution contexts (`process`, `require`, `global`) must be proactively added to the blocked identifier lists.
