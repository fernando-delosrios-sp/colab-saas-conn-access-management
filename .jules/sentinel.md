...
## 2024-05-24 - [SSTI via Velocity AST Dynamic Index Bypass]
**Vulnerability:** Attackers could bypass basic Velocity AST validation blocking direct string literals (like `"constructor"`) by using `#set` directives to dynamically construct the dangerous string via concatenation (e.g. `#set($c = "con" + "structor")`) and then using it as an index accessor (`$foo[$c]`).
**Learning:** Security validations on ASTs must simulate or trace variable states dynamically. Simply checking for string literals across the entire AST is insufficient and leads to bypasses if string operations are allowed. Furthermore, globally blocking terms like `"constructor"` can cause false positives (e.g., `#set($role = "constructor")`), necessitating context-aware validation.
**Prevention:** Implement context-aware AST traversal that tracks variables defined in `#set` directives, evaluates simple string concatenations, and explicitly checks the resolved values only when they are used in sensitive contexts (like `index`, `property`, or `method` accesses).
