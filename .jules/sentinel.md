...
## 2024-05-24 - VelocityJS AST Sandbox Bypass via Prototype Pollution and Macro Execution

**Vulnerability:** The `isUnsafeVelocityAST` function implemented to sandbox VelocityJS templates only blocked `constructor` and `__proto__`. It missed `prototype` and macro executions like `#evaluate()`, allowing attackers to perform SSTI and execute arbitrary code.
**Learning:** AST-based sandboxes must be comprehensive. When restricting property accesses and indexing, all vectors for accessing the object prototype chain must be considered, including `prototype` strings. Additionally, evaluate macros provided by template engines need to be disabled or strictly filtered to prevent dynamically executing malicious strings.
**Prevention:** Explicitly block `__proto__`, `prototype`, `constructor`, and index-based property accesses using these specific dangerous strings. Also block macro evaluations like `#evaluate`. Avoid blocking all non-integer indexing to maintain valid template functionality (e.g. Map variable accesses).
