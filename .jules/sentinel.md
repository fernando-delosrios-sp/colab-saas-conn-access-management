...
## 2024-08-11 - Block Velocity Prototype Pollution bypasses
**Vulnerability:** Attackers can bypass basic Prototype Pollution validation in Velocity templates by using `#set` string concatenation (e.g. `#set($c = "con" + "structor")`) and dynamic index access.
**Learning:** Basic AST validation against literal identifier names is insufficient in templates that support variable assignment and string manipulation.
**Prevention:** Implement static evaluation in AST validation to trace variable assignments and calculate string concatenations prior to runtime evaluation.
