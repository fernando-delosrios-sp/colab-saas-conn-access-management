...
## 2025-02-28 - [Velocity Template Sandbox Escape Mitigation via String Concatenation Bypass]

**Vulnerability:** The existing validation for Velocity templates in `src/utils/index.ts` failed to prevent AST-based Sandbox Escapes or Prototype Pollution in `velocityjs` when strings were dynamically constructed via concatenation in `#set` directives (e.g., `#set($c = "con" + "structor") $foo[$c]`).
**Learning:** In `velocityjs`, inline string concatenation within index accessors results in a native parse error. However, attackers can bypass basic AST validation by concatenating strings via `#set` directives and accessing them dynamically (`$foo[$c]`).
**Prevention:** To prevent SSTI, use a static evaluation mechanism to trace variable assignments and calculate simple string concatenations (using the `+` operator) during AST analysis, blocking them when used in sensitive contexts like `index` node references.
