...
## 2025-05-15 - [Bypass of VelocityJS AST validation via variable concatenation]
**Vulnerability:** Attackers could bypass basic SSTI AST validation by concatenating dangerous strings (like "con" + "structor") using `#set` directives and accessing them dynamically (`$foo[$c]`).
**Learning:** AST validation must be context-aware. Statically evaluating simple arithmetic and tracking variable states is required to prevent obfuscated payloads in sandboxed template engines.
**Prevention:** Track `#set` assignments and compute basic string concatenations during AST traversal, validating the final resolved values when used as dynamic object indexes.
