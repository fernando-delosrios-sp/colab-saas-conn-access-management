...
## 2025-03-08 - SSTI Prevention in Velocity AST via Variable Tracking
**Vulnerability:** Attackers could bypass static AST checks for dangerous properties (`constructor`, `__proto__`) by dynamically concatenating strings via `#set` directives and accessing them dynamically (e.g., `#set($c = "con" + "structor")\n$foo[$c]`).
**Learning:** Simple AST validation is not sufficient against SSTI in velocity. Blocking direct identifier strings globally causes false positives, while ignoring variable resolution allows bypassing checks via concatenation and dynamic indexing.
**Prevention:** Validation must be context-aware. Track dangerous variables defined in `#set` directives using static evaluation of basic concatenations, and exclusively block their usage (or direct strings) when used as dynamic `index`, `property`, or `method` accessors.
