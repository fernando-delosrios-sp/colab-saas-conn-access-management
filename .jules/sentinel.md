...
## 2025-02-27 - [Velocity JS Sandbox Escape via Dynamic Evaluation]
**Vulnerability:** A sandbox escape and prototype pollution vulnerability existed in the Velocity JS template evaluation. While explicit string access to dangerous properties like `['constructor']` was blocked, attackers could bypass this using dynamic evaluation via variables, inline eval (`["#evaluate('constructor')"]`), or math expressions within index nodes.
**Learning:** Checking for static dangerous strings in AST is insufficient for security in template engines. Velocity AST allows multiple ways to dynamically resolve property names, bypassing static string checks.
**Prevention:** When validating `velocityjs` ASTs, explicitly block dynamic evaluation bypasses by checking for properties like `isEval === true`, `type === 'references'`, and `type === 'math'` on index nodes. This prevents Sandbox Escapes and Prototype Pollution via dynamically constructed unsafe keys.
