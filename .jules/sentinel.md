...
## 2025-02-28 - [Server-Side Template Injection Evasion via Velocity Index AST Lookup]

**Vulnerability:** Even with AST validation checking for explicit string keys like `constructor` and `__proto__`, the Velocity template engine evaluator allows dynamic property access via `[$var]` (e.g. `$user[$var]`) or concatenated evaluation. Because the variables inside the index are only evaluated at runtime, static AST analysis that solely checks literal index strings (e.g., `$user['constructor']`) misses malicious payloads dynamically generated in templates. This allows attackers to bypass the sandbox and achieve Remote Code Execution.
**Learning:** AST validation for template engines cannot trust dynamic accessors (indexes). An attacker can hide banned property names in variables or concatenate them during runtime to evade literal string checks on the AST nodes.
**Prevention:** To prevent evasion, strictly enforce that all AST `index` nodes use only static types (such as `string` or `integer`). Block any node types used as indexes that are not literals (e.g., blocking `references` inside an index like `$user[$var]`) so dynamic lookup bypasses are impossible. Additionally, explicitly check naked `references` nodes matching banned keywords (e.g., `$process`).
