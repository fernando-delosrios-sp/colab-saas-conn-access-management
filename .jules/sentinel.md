...
## 2025-02-18 - Prevent SSTI by Validating AST Context and Dynamic Index Accessors
**Vulnerability:** Attackers could bypass basic AST validation for Server-Side Template Injection (SSTI) by concatenating strings via `#set` directives and accessing them dynamically (`$foo[$c]`), leading to Prototype Pollution or remote code execution.
**Learning:** String concatenation within index accessors results in native parse errors in `velocityjs`, but dynamic references evaluate perfectly. To prevent SSTI, validation must be context-aware, evaluate `#set` variable string concatenations statically, and block sensitive strings (`constructor`, `__proto__`, `prototype`) in `property`, `method`, and evaluated `index` node references.
**Prevention:** Always statically analyze AST variables set by macros and explicitly evaluate their concatenated string value when checking for potentially dangerous identifier access in dynamic templates.
