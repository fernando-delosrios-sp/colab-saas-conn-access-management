...
## 2025-03-01 - SSTI Bypass in Velocity AST Validation via String Concatenation
**Vulnerability:** A critical Server-Side Template Injection (SSTI) vulnerability in the Velocity template validation logic allowed attackers to bypass identifier checks (for `constructor`, `__proto__`, `prototype`) by dynamically computing string accessors via index concatenation in `#set` directives (e.g. `#set($c = "con" + "structor")` -> `$foo[$c]`).
**Learning:** Basic AST analysis fails when evaluating dynamic execution paths where properties are constructed at runtime, demonstrating that strict identifier denylists are fragile if they don't trace variable state.
**Prevention:** Velocity validation rules must statically trace variable assignments and evaluate arithmetic string operations (`+`) within `#set` directives to capture dynamic references during validation, before execution. Also, block properties and methods specifically to prevent context evasion.
