...
## 2025-02-20 - [VelocityJS AST Sandbox Escape & Prototype Pollution]

**Vulnerability:** The `velocityjs` template parser AST validation (`isUnsafeVelocityAST`) only blocked static string access to `constructor` and `__proto__`. It did not account for `prototype`, dynamic/evaluated string concatenation indexing (`$foo["con" + "structor"]`), dynamic references (`$foo[$bar]`), or the `#evaluate` macro (`#evaluate("some string")`), allowing an attacker to bypass the checks and execute arbitrary code (Sandbox Escape) or pollute the prototype.
**Learning:** AST-based security checks must be exhaustive. Relying on an explicit blocklist of exact string values is insufficient because dynamic languages allow strings and keys to be constructed or referenced indirectly (e.g., via variable references or expression evaluation).
**Prevention:** When validating an AST for unsafe property access, block all dynamic evaluations and non-literal lookups. Require strictly safe types (e.g., block all `macro_call` for `evaluate` and allow only `integer` types for array indexing) rather than just blocking specific string values.
