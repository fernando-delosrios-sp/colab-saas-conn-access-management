...
## 2024-06-27 - [Server-Side Template Injection via Velocity String Concatenation Bypass]

**Vulnerability:** The AST-based validation for Velocity templates correctly blocked direct access to `constructor`, `__proto__`, and `prototype` in identifiers and static string indices. However, an attacker could bypass this by concatenating strings using math operators in a `#set` directive (e.g., `#set($c = "con" + "structor")`) and then passing that variable dynamically as an index (`$foo[$c]`).
**Learning:** Checking for bad words in static strings is insufficient if the template language allows evaluating simple operations and variables before dynamic access. Attackers can obfuscate restricted keywords via string concatenation (`+`) to build the restricted property names at runtime.
**Prevention:** To prevent SSTI through obfuscation, AST validation must statically trace variable assignments (e.g., `#set`). It must evaluate basic math operations on strings during validation, map the assigned values to their reference identifiers, and ensure these variables are not used in sensitive contexts like dynamic `index` nodes.
