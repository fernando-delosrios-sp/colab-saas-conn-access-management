...
## 2025-02-28 - SSTI bypass via variable-based index access in Velocity
**Vulnerability:** Template expressions like `$entitlement[$var]` or `$entitlement["con" + "structor"]` bypassed static AST checking, leading to possible Server-Side Template Injection / Prototype Pollution.
**Learning:** Static property checking by matching exact literal string names is not sufficient if the AST parser allows expressions or variables inside index accessors.
**Prevention:** Block non-literal index access entirely in velocityjs templates and explicitly expand the list of banned property identifiers to include Node.js environment specifics like `process`, `require`, and `global`.
