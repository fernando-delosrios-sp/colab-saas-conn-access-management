...
## 2025-02-28 - [Server-Side Template Injection via Velocity Templates (Assignment Block)]

**Vulnerability:** The existing validation for Velocity templates in `src/utils/index.ts` blocked string-based prototype access, but failed to block the `#set` directive (`nodes.type === 'set'`). This allowed attackers to perform dynamic variable assignments, enabling the construction of complex payloads (like Server-Side Template Injection, or SSTI) that could bypass standard string literal filters by dynamically generating or concatenating restricted strings within the template context.
**Learning:** Preventing access to `constructor` or `__proto__` is insufficient if the template engine still permits variable assignment. Attackers can leverage variable assignments to construct restricted properties iteratively or dynamically, rendering simple string blocklists ineffective.
**Prevention:** Always explicitly block all dynamic variable assignments (e.g., `#set` in Velocity, or assignment operators in other engines) during AST validation to prevent attackers from constructing malicious payloads in the rendering context.
