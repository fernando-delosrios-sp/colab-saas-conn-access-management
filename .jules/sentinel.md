...
## 2025-02-28 - [Velocity Template Sandbox Escape via String Concatenation in #set]
**Vulnerability:** Attackers could bypass static string matching in Velocity AST validation by assigning dynamic string concatenations (e.g., `"con" + "structor"`) to variables using `#set` directives, and then using those variables for dynamic index access (`$foo[$c]`). This allows executing arbitrary code by accessing prototype functions.
**Learning:** Naive static string checks on AST nodes are insufficient. Attackers can leverage the template engine's native variable assignment and operators to hide malicious strings. Static analysis must track basic variable state to effectively prevent Server-Side Template Injection (SSTI).
**Prevention:** Update AST validation to pass a mutable environment object during traversal. Evaluate `#set` string literals and basic concatenations, and substitute these evaluated values when inspecting index reference identifiers.
