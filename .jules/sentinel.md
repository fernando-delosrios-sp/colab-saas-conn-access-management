...
## 2025-02-28 - [Velocity Template Macro and Dynamic Index Evaluation Bypass]

**Vulnerability:** The previous AST-based validation for Velocity templates blocked `constructor` and `__proto__` when accessed via direct identifiers or static string index access (e.g., `['constructor']`). However, it missed blocking access to the `prototype` property. More critically, it failed to block dynamic evaluation bypasses, such as macro evaluations (`#evaluate("$obj.constructor")`) and dynamic string/variable index evaluations (e.g., `#set($a = 'constructor') $obj[$a]`) which allowed attackers to bypass the sandbox and execute arbitrary code via SSTI.
**Learning:** Checking only simple string values in index access is insufficient. We must block dangerous macro features like `#evaluate` and specifically target dynamic index evaluations where string or reference types resolve to forbidden keys (e.g. `$constructor`, `$prototype`, `$__proto__`). We must also ensure we don't arbitrarily block all non-integer indexing (which breaks legitimate string map lookups).
**Prevention:** In AST-based validation for Velocity templates, explicitly block the `prototype` property, block the `evaluate` macro, and explicitly block `index` nodes where the identifier string or variable reference resolves to the forbidden strings.
