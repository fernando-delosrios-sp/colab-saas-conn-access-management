...
## 2025-07-02 - [Server-Side Template Injection Bypass via String Concatenation]

**Vulnerability:** The existing Velocity AST validation for SSTI only checked explicit string values in index accessors. Attackers bypassed this by assigning dynamically concatenated strings (e.g. `#set($c = "con" + "structor")`) to variables, which were then used as keys (e.g. `$foo[$c]`) to access forbidden properties like `constructor`.
**Learning:** Checking hardcoded string nodes in AST index references is insufficient because template engines natively evaluate variables and expressions. If string concatenations aren't evaluated statically during AST traversal, the sandbox can be easily bypassed.
**Prevention:** Enhance the AST validation logic to act as a partial static analyzer that tracks variable assignments from `#set` directives. Compute simple math operators (like `+` for strings) at parse time and substitute those variables when checking dynamic index accessors.
