...
## 2024-08-09 - [Fix SSTI bypass via string concatenation]
**Vulnerability:** Attackers can bypass basic AST validation blocking `constructor`/`__proto__` by concatenating strings in `#set` directives and accessing them dynamically (e.g., `#set($c = "con" + "structor")\n$foo[$c]`).
**Learning:** Basic node identifier checks in velocityjs ASTs are insufficient for preventing Prototype Pollution or SSTI; static evaluation must trace variable assignments and simple concatenations.
**Prevention:** Always implement context-aware tracing for variables defined within template scopes and evaluate string interpolations before verifying access safety.
