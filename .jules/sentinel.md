...
## 2025-03-01 - [Velocity SSTI Bypass via String Concatenation]
**Vulnerability:** Attackers could bypass basic AST validation for Server-Side Template Injection (SSTI) in Velocity templates by concatenating strings in `#set` directives (e.g., `#set($c = "con" + "structor")`) and accessing them dynamically (`$foo[$c]`).
**Learning:** In `velocityjs`, inline string concatenation in index accessors (e.g., `$foo["con" + "structor"]`) throws native parse errors, but `#set` bypasses this. Sandboxing must trace assignments and evaluate simple string operations during AST analysis to block sensitive properties correctly.
**Prevention:** Use a static evaluation mechanism during AST validation (`isUnsafeVelocityAST`) to track variable assignments, evaluate basic concatenations (`+`), and block their evaluation when used in dynamic index node references.
