...
## 2025-02-28 - [Server-Side Template Injection Bypass via String Concatenation]

**Vulnerability:** The existing `isUnsafeVelocityAST` check to prevent SSTI in `velocityjs` templates was bypassing strict literal matches by failing to validate concatenated strings or aliases created via `#set` directives. For example, attackers could use `#set($c = "con" + "structor")` followed by dynamic index access like `$object[$c]`, which evaded simple literal identifier checks.
**Learning:** Checking strictly for literal properties or string identities in abstract syntax trees (ASTs) is insufficient when dynamic macros or variable string assignments allow concatenation. The AST often parses concatenated values as math operations or string assignments.
**Prevention:** In addition to checking identifiers or direct property paths against a banned list, evaluate string combinations statically through AST node traversal (e.g., evaluating `"con" + "structor"`) to ensure dynamically built properties cannot target banned prototype or global attributes (`__proto__`, `constructor`, `prototype`, `process`, `require`, `global`).
