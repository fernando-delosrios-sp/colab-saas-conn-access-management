...
## 2025-02-14 - AST Injection Vulnerability in Velocity Templates
**Vulnerability:** The application uses VelocityJS to evaluate templates based on user-supplied or external data. The previous validation function only prevented access to `constructor` and `__proto__`, but it was missing protection against accessing `prototype` and using potentially dangerous macro calls like `#evaluate`.
**Learning:** Checking for common Prototype Pollution vectors isn't sufficient for sandbox execution engines. In Velocity, `#evaluate` directly executes strings as code, leading to Remote Code Execution (RCE) / Sandbox Escapes if an attacker can control the template structure. Furthermore, prototype pollution protections must include the `prototype` property in addition to `__proto__` and `constructor`.
**Prevention:** Thoroughly analyze the features of third-party templating engines. When using an Abstract Syntax Tree (AST) validator, explicitly block arbitrary execution macros (`#evaluate`) and all forms of prototype manipulation (`__proto__`, `constructor`, `prototype`). Always build tests checking for these specific bypass vectors.
