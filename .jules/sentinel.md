...
## 2025-02-28 - [Velocity SSTI Bypass via String Concatenation]
**Vulnerability:** Attackers could bypass basic AST validation by concatenating strings via `#set` directives (e.g., `#set($c = "con" + "structor")`) and accessing them dynamically (`$foo[$c]`), leading to Server-Side Template Injection (SSTI).
**Learning:** Basic AST validation that only checks for exact string matches in accessors can be bypassed by dynamically constructing the dangerous string.
**Prevention:** The validation logic must statically evaluate simple string concatenations in `#set` directives during AST analysis to track dangerous variables and block them when used in sensitive contexts like `index` node references.
