...
## 2024-05-15 - SSTI / Prototype Pollution bypass in Velocity templates
**Vulnerability:** Attackers could bypass Velocity template AST validation that blocks dangerous keywords (`constructor`, `__proto__`, `prototype`) by constructing strings dynamically using `#set` assignments and string concatenation (e.g., `#set($c = "con" + "structor")` followed by `$obj[$c]`). Furthermore, checking strings globally caused false positives on safe assignments like `#set($role = "constructor")`.
**Learning:** Static string checking is insufficient for template engines that support variables and string manipulation. Validation must be context-aware, track variable assignments dynamically, and strictly validate property/method access nodes and evaluated index references.
**Prevention:** When validating ASTs for Server-Side Template Injection (SSTI) or Prototype Pollution, implement a static evaluator that tracks simple variable assignments and operations. Ensure validation focuses on the context of execution (e.g., property access, method calls, and dynamic index resolution) rather than blindly blocking strings across the entire AST.
