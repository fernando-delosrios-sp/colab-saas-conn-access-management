...
## 2025-02-28 - [Server-Side Template Injection via Velocity Templates (String Concatenation Bypass)]
**Vulnerability:** Attackers could bypass the AST-based sandbox validation in `isUnsafeVelocityAST` by concatenating strings using `#set` assignments (e.g. `#set($c = "con" + "structor")`) and accessing them dynamically (`$foo[$c]`).
**Learning:** Basic AST validation that only checks static index accesses can be easily bypassed by variable assignments and simple string concatenation operations inside the template itself.
**Prevention:** The validation logic must be context-aware by maintaining a map of variable assignments, statically evaluating simple string operations like concatenations (`+`) during AST analysis, and blocking dynamically calculated strings when used in index accessors.
