...
## 2024-05-18 - SSTI bypass via concatenated string logic in index accessors
**Vulnerability:** In `velocityjs`, inline string concatenation bypasses basic AST validation. Attackers can concatenate strings via `#set` directives and access them dynamically.
**Learning:** Basic AST validation that only checks for direct string literals in index accessors is insufficient. Dynamic evaluation of variables defined in `#set` directives is required.
**Prevention:** Use a static evaluation mechanism to trace variable assignments and calculate simple string concatenations (`+`) during AST analysis, blocking them when used in sensitive contexts like `index` node references.
