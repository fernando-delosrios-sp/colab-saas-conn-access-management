...
## 2024-07-05 - [Fix velocityjs Template AST Validation]
**Vulnerability:** Sandbox Escape / Prototype Pollution in `velocityjs` templates via dynamic property evaluation.
**Learning:** Checking the parsed AST for direct string literals (like `id.value === 'constructor'`) is insufficient when `velocityjs` permits dynamic property references (e.g. `$foo[$bar]`, `#set($fn = $foo[$bar])`). The parser sets `isEval: true`, `type: 'references'`, or `type: 'math'` on index nodes in these dynamic cases.
**Prevention:** Always block dynamic evaluations (`isEval: true`, `type: 'references'`, `type: 'math'`) on property accesses and explicitly block `prototype` in addition to `__proto__` and `constructor` to comprehensively prevent prototype pollution and sandbox bypasses in AST validators.
