...
## 2024-08-25 - Prevent SSTI Evasion in Velocity Templates
**Vulnerability:** Attackers could bypass basic AST validation blocking `constructor`, `__proto__`, and `prototype` by concatenating strings in `#set` directives (e.g., `#set($c = "pro" + "totype")`) and accessing them dynamically (`$foo[$c]`).
**Learning:** Validation for template injection must trace variable assignments and calculate static string manipulations to secure context-aware identifier checks in dynamic index node references.
**Prevention:** Use a static evaluator for simple string concatenation in AST parsing and exclusively block banned references when used as dynamic index accessors.
