...
## 2024-08-23 - VelocityJS AST Bypass via Dynamic String Concatenation
**Vulnerability:** Attackers could bypass basic AST blocklists (which block `constructor`, `__proto__`, etc.) by dynamically concatenating strings in `#set` directives (e.g., `#set($c = "con" + "structor")`) and evaluating them as index properties (`$foo[$c]`).
**Learning:** Checking for unsafe identifiers statically is insufficient when template languages allow arbitrary local variable assignment. Validations must track variable scope (context-aware tracking) during AST traversal to resolve dynamic concatenations used in sensitive object property access modes (`index`, `property`, `method`).
**Prevention:** Always emulate and track variable assignments through directives (like `#set` in Velocity) and compute statically resolvable string concatenations during analysis, evaluating those derived values against the blocklist when used in access nodes.
