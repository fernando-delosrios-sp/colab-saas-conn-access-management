...
## 2024-05-27 - [Bypass AST validation with string concatenation]
**Vulnerability:** Velocity AST validation was bypassed by using string concatenation in `#set` directives to construct dangerous identifiers, which were then used as dynamic index accessors.
**Learning:** AST validation for template injection must statically trace variable assignments and calculate simple string operations to detect hidden dangerous identifiers.
**Prevention:** Track string variable assignments during AST traversal and evaluate them contextually when they are used as node references (like `index`).
## 2026-09-24 - [Package Manager Engine Version Incompatibility]
**Vulnerability:** The `npm ci` build in GitHub Actions failed due to an engine incompatibility: `velocityjs@2.1.7` requires Node >= 20.19.0, but the workflow ran on an older version of Node (18.20.8) and NPM (10.8.2). This was caused by the `^2.1.5` semver range in `package.json` automatically pulling in the latest minor/patch release (2.1.7) during `npm ci` without a valid `package-lock.json` lock.
**Learning:** Using permissive dependency ranges (like `^` or `~`) without committing `package-lock.json` or pinning exact versions can introduce unexpected breaking changes or engine requirement conflicts during CI builds when transitive dependencies publish new versions.
**Prevention:** Always lock dependencies by either pinning exact versions in `package.json` (e.g., removing `^`) or ensuring `package-lock.json` is consistently committed and updated. For this fix, the range was restricted to `~2.1.5` to prevent fetching `2.1.7` while maintaining patch updates, resolving the Node engine mismatch on CI.
