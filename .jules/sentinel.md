...
## 2024-05-18 - [Fix Server-Side Template Injection in Velocity Templates]
**Vulnerability:** The initial Velocity AST validation failed to detect dangerous identifiers (e.g. `constructor`, `__proto__`) when constructed dynamically via `#set` assignments and string concatenations, leading to Server-Side Template Injection (SSTI).
**Learning:** In dynamically evaluated templates, blocking raw string identifiers in index accessors is insufficient. Attackers can bypass this by assigning concatenated strings to variables and passing those to dynamic indexers. The validation logic must simulate static evaluation of variables.
**Prevention:** Ensure AST validators recursively track environment state for assignments (`#set`) and compute basic literal expressions (string concatenation) when scanning for unsafe method or property lookups.
