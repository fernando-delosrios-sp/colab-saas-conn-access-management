...
## 2025-02-28 - [Server-Side Template Injection via Velocity Templates (Update 2)]

**Vulnerability:** The existing `hasConstructor`/`isUnsafeVelocityAST` validation for Velocity templates only blocked identifiers and simple index lookups. It failed to prevent access to banned properties when the property name was dynamically constructed via `#set` directives and concatenated strings (e.g., `#set($c = "con" + "structor")` followed by `$object[$c]`). This allowed attackers to easily evade static AST checks and achieve Server-Side Template Injection (SSTI).
**Learning:** AST-based validation for template engines must explicitly check for dynamic string construction (e.g., `math` operations in `#set` directives and `index` accessors) because attackers can construct banned property names at runtime to bypass basic literal checks.
**Prevention:** The validation logic in `isUnsafeVelocityAST` must be updated to statically evaluate `math` nodes within `#set` assignments and `index` properties across the AST to detect and block constructed banned properties (`constructor`, `__proto__`, `prototype`, `process`, `require`, `global`).
