...
## 2025-02-28 - [Sandbox Escape bypass with non-literal index accessors in Velocity Templates]

**Vulnerability:** The existing `hasConstructor` validation for Velocity templates in `src/utils/velocity.ts` blocked access to `constructor`, `__proto__`, and `prototype` but allowed dynamic evaluation and AST manipulation bypasses using variables and mathematical concatenations in index properties, e.g., `$foo[$var]` or `#set($val = 'con' + 'structor')\n$foo[$val]`. This could allow attackers to bypass literal checks and execute code on the server.
**Learning:** Checking strict string equivalence of index property names without inspecting the types of indices in a template AST is insufficient because an attacker can hide the banned strings inside variables or concatenated operations that avoid simple literal matches in the AST.
**Prevention:** Explicitly block all non-literal index accessors (`type !== 'string' && type !== 'integer'`) alongside explicit checks for banned string literals (`['constructor', '__proto__', 'prototype', 'process', 'require', 'global']`). This restricts evaluation solely to static and known property accesses.
