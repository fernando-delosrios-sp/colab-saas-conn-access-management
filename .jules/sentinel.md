...
## 2025-02-18 - Fix SSTI / Prototype Pollution bypass in Velocity AST checking
**Vulnerability:** The existing Velocity AST validation blocked direct literal access to `constructor` or `prototype`, but it failed to block non-literal index lookups (`$var` or concatenated strings). This allowed attackers to bypass the AST checks by dynamically referencing prohibited properties, which could lead to Sandbox Escape or Remote Code Execution. In addition, identifiers like `process`, `require`, and `global` were not strictly blocked.
**Learning:** Checking the literal AST structure isn't sufficient if the templating engine allows variable resolution during evaluation. Dynamic index lookups are a common evasion vector for Prototype Pollution and SSTI.
**Prevention:** Always block dynamic / non-literal index lookups when enforcing strict AST structure checks. Ensure that Node's environment-specific primitives (`process`, `require`, `global`) are also explicitly disallowed.
