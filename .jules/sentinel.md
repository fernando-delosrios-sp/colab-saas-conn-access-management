...
## 2025-02-23 - Prevent Velocity JS SSTI string concatenation bypasses
**Vulnerability:** Server-Side Template Injection (SSTI) bypass via `#set` directives and string concatenation.
**Learning:** Attackers can bypass basic AST validation by concatenating strings via `#set` directives (e.g., `#set($c = "con" + "structor")`) and accessing them dynamically (`$foo[$c]`).
**Prevention:** Use a static evaluation mechanism to trace variable assignments and calculate simple string concatenations (using the `+` operator) during AST analysis, blocking them when used in sensitive contexts like `index` node references.
