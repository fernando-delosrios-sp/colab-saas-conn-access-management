...
## 2025-02-14 - Fix advanced SSTI bypasses in velocity templates
**Vulnerability:** Advanced Server-Side Template Injection (SSTI) / Prototype Pollution bypassing via variable assignment and string concatenation. The AST validation previously checked direct identifiers globally, allowing attackers to use `#set($c = "con" + "structor")` and `$entitlement[$c]` to bypass validation entirely.
**Learning:** In velocityjs, standard index accessors (`$entitlement["con" + "structor"]`) often fail basic parsers or can be blocked, but using `#set` bindings effectively obfuscates the dangerous string. The AST must be statically evaluated to resolve concatenated strings (`math` node with `+`) and track variables across statements to validate dynamic index nodes properly. Additionally, a global block of `constructor` causes false positives (e.g., `#set($role = "constructor")`); it should only be blocked when used in `index` or `property` accesses.
**Prevention:** Track `#set` target variables by locally evaluating strings and math expressions within the AST. When checking `index` nodes, lookup the dynamic variable ID to see if it resolves to a dangerous string like `constructor` or `__proto__`. Check properties specifically, instead of blocking the string globally across the AST.
