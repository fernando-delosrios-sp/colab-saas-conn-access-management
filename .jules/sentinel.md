...
## 2024-05-24 - Fix Velocity SSTI Bypass via String Concatenation
**Vulnerability:** Velocity AST validation bypassed dynamic index accessors when string concatenation was used within `#set` directives (e.g., `#set($c = "con" + "structor")`).
**Learning:** Basic AST validation against raw string values is insufficient because it doesn't account for state changes across directives. String concatenations via `#set` can hide dangerous payloads during static analysis.
**Prevention:** AST validation must be stateful, tracking variables declared in templates and statically evaluating simple operations like string concatenations to ensure dynamic accessors do not resolve to forbidden properties.
