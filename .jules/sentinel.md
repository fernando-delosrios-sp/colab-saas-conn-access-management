...
## 2025-02-28 - [Server-Side Template Injection via Dynamic Index Access]

**Vulnerability:** The `isUnsafeVelocityAST` sandbox for Velocity templates successfully blocked literal property access (like `$object.constructor` or `$object["constructor"]`) but failed to inspect dynamic or non-literal index accessors. Attackers could evade AST-based literal string checks by supplying a variable (e.g., `$object[$var]` where `$var` is `constructor`) or by concatenating strings at runtime (e.g., using macro Math operations), achieving Server-Side Template Injection (SSTI).
**Learning:** In dynamically typed templating engines (like `velocityjs`), relying solely on AST literal checks for sandbox security is insufficient. Attackers will use dynamic variable resolution or expression evaluation within index accessors to hide banned identifiers (e.g., `constructor`, `__proto__`, `process`) from static AST string checks.
**Prevention:** Explicitly block *all* non-literal or dynamic index accessors (e.g., variables, expressions) within AST parsing by throwing an error if `id.type` is not a `string` or `integer`. Also ensure `process`, `require`, and `global` are explicitly banned along with prototype poisoning properties.
