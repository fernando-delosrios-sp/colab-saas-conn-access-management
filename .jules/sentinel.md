...
## 2026-07-27 - [Fix SSTI / Sandbox Escape in velocity template evaluation]
**Vulnerability:** The application was vulnerable to Server-Side Template Injection (SSTI) and Sandbox Escapes because the strict AST validation for `velocityjs` templates was incomplete. While direct access to `constructor` and `prototype` was blocked, attackers could bypass this validation by accessing these properties dynamically (e.g. via dynamic index accessors like `$foo[$var]` or `$foo["con" + "structor"]`) and via `process`, `require`, and `global`.
**Learning:** When securing `velocityjs` templates against SSTI/Prototype Pollution, strict AST validation must explicitly block dangerous identifiers (`__proto__`, `prototype`, `constructor`, `process`, `require`, `global`) and macro evaluations (`#evaluate`). Crucially, dynamic index accessors can be used to evade literal AST checks by hiding banned properties in variables or concatenating them at runtime.
**Prevention:** To prevent sandbox escapes, explicitly block non-literal (e.g., non-string/non-integer) index lookups in AST validation and explicitly block dangerous properties on those literals.
