...
## $(date +%Y-%m-%d) - Optimize Redundant Map Lookups in Inner Loops

**Learning:** When iterating through a Map in performance-sensitive inner loops, using `Map.keys()` and then subsequently calling `Map.get(key)` inside the loop creates redundant O(1) lookups that accumulate overhead over thousands of iterations. Furthermore, re-evaluating loop-invariant assignments (such as config resolution or boolean evaluations) inside the inner loop adds unnecessary processing time.
**Action:** Always iterate using `Map.entries()` (`for (const [key, value] of map.entries())`) to retrieve the key and value simultaneously, avoiding redundant `.get()` lookups. Additionally, always hoist loop-invariant assignments and condition checks outside of inner loops to prevent redundant execution.
