...
## 2026-07-06 - Batch Pre-fetching Sources and Apps

**Learning:** Despite using a concurrency limiter for API calls (`processConcurrent`), looping through arrays to fetch individual entities via API (e.g., `getAppByName` and `getSource`) can still lead to significant network overhead and API rate limits when iterating over hundreds or thousands of entitlements.
**Action:** Replace concurrent individual API calls with batched API calls using the `in` filter (e.g., `name in ("xyz", "abc")` or `id in ("123", "456")`). This significantly reduces the number of API requests, preventing rate limits and reducing the total network roundtrip time.
