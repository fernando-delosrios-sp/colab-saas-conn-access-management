...
## 2024-05-15 - [Avoid premature loop caching and reference sharing]
**Learning:** Caching parsed ASTs inside a loop where the expression generation relies on iteration-specific context (e.g. `groupName`) is an anti-pattern as the cache will always miss. Furthermore, using `JSON.parse(JSON.stringify())` to deep clone objects in a hot loop to avoid shared reference bugs creates severe memory and CPU overhead, defeating the purpose of the optimization. Avoid hoisting variables that return objects (like `accessRequestConfig`) unless you can safely mutate them, as they create unintended shared references across iteration entities. Additionally, do not hoist property access that is already guarded by loop conditions (e.g., skip operations during delete mode) as it can introduce TypeErrors.
**Action:** Optimize loops by using optimal iteration methods (like `Map.entries()` over `Map.keys()`), completely avoiding deep cloning serialization in hot paths, and ensuring any hoisted variable assignments are both unconditionally executed and truly invariant.
