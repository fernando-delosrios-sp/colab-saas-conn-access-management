...
## 2026-07-25 - Pre-fetch entitlements for role definition queries concurrently
**Learning:** During role processing, iterating sequentially over role definitions and executing API queries inside the loop (`await isc.listEntitlements(definition.query)`) creates a massive N+1 query bottleneck, exacerbating network latency and slowing down execution.
**Action:** Always pre-fetch nested dependencies in batches using a concurrency limiter (like `runWithConcurrency`) before entering inner loops, caching the results in a Map or Set to replace network I/O with O(1) memory lookups.
