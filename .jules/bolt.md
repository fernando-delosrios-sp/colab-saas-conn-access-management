...
## 2025-02-18 - [Prevent Rate Limits with Concurrency Limiter]
**Learning:** Unbounded `Promise.allSettled` calls for processing large datasets in API operations can trigger HTTP 429 rate limit errors and socket exhaustion.
**Action:** Always replace unbounded `Promise.allSettled` with a concurrency limiter like `runWithConcurrency`, wrapping the inner logic in `try/catch` to return an object mimicking `PromiseSettledResult` to maintain compatibility with downstream consumers.
