...
export const connector = async () => {
    ...
    const stdEntitlementList: StdEntitlementListHandler = async (context, input, res) => {
        ...
        try {
            // ⚡ Bolt: Pre-fetch all distinct entitlements globally to eliminate redundant API calls across configurations
            const globalQueries = new Set<string>()
            if (config.accessProfiles) config.accessProfiles.forEach((d) => globalQueries.add(d.query))
            if (config.roles) config.roles.forEach((d) => globalQueries.add(d.query))

            const queryToEntitlementsMap = new Map<string, EntitlementV2025[]>()
            if (globalQueries.size > 0) {
                logger.debug(`Pre-fetching entitlements for ${globalQueries.size} globally unique queries`)
                const queryResults = await processConcurrent(Array.from(globalQueries), async (query) => {
