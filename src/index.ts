        ...
        try {
            ...
            if (config.accessProfiles) {
                ...
                accessProfiles: for (const definition of config.accessProfiles) {
                            ...
                            if (definition.groupType === 'application') {
                                ...
                                if (attributeName) {
                                    ...
                                    if (attributeValue) {
                                        ...
                                        logger.error(
                                            `Entitlement ${entitlement.name} has no ${attributeName} attribute`
                                        )
                                ...
                                if (name) {
                                    logger.debug(
                                        `Grouping by attribute ${definition.groupAttribute} with value ${name}`
                                    )
                    ...
                    // ⚡ Bolt: Fetch source and app data concurrently with limits to prevent rate limits
                    const sourcePromises = processConcurrent(Array.from(sourcesToFetch), async (sId) => {
                        const source = await isc.getSource(sId)
                        if (source.owner?.id) {
                            sourceOwnerMap.set(sId, source.owner.id)
                        }
                    })
                    const appPromises = processConcurrent(Array.from(appsToFetch), async (appName) => {
                        const app = await isc.getAppByName(appName)
                        return { appName, app }
                    })

                    const [_, fetchedApps] = await Promise.all([sourcePromises, appPromises])
                ...
                // ⚡ Bolt: Fetch existing access profiles concurrently but chunked to prevent API rate limits
                ...
                // ⚡ Bolt: Create/update access profiles in limited batches
                await processConcurrent(Array.from(accessProfileMap.entries()), async ([apName, ap]) => {
                ...
                // ⚡ Bolt: Create/update applications in limited batches
                await processConcurrent(Array.from(applicationMap.entries()), async ([appName, app]) => {
            ...
            if (config.roles) {
                ...
                // ⚡ Bolt: Fetch existing roles concurrently but chunked to prevent API rate limits
                ...
                // Create/update roles
                // ⚡ Bolt: Create/update roles in limited batches
                await processConcurrent(Array.from(roleMap.entries()), async ([roleName, role]) => {
