        ...
        try {
            ...
            if (config.accessProfiles) {
                ...
                accessProfiles: for (const definition of config.accessProfiles) {
                    ...
                    // ⚡ Bolt: Use processConcurrent to avoid rate limits when pre-fetching
                    const _ = await processConcurrent(Array.from(sourcesToFetch), 10, async (sId) => {
                        const source = await isc.getSource(sId)
                        if (source.owner?.id) {
                            sourceOwnerMap.set(sId, source.owner.id)
                        }
                    })
                    const fetchedApps = await processConcurrent(Array.from(appsToFetch), 10, async (appName) => {
                ...
                const existingAps = await processConcurrent(apNames, 10, (name) => isc.getAccessProfileByName(name))
                ...
                // ⚡ Bolt: Use processConcurrent with limit 10 instead of unbounded Promise.all
                await processConcurrent(Array.from(accessProfileMap.entries()), 10, async ([apName, ap]) => {
                ...
                // ⚡ Bolt: Use processConcurrent with limit 10 instead of unbounded Promise.all
                await processConcurrent(Array.from(applicationMap.entries()), 10, async ([appName, app]) => {
            ...
            if (config.roles) {
                ...
                const existingRoles = await processConcurrent(roleNames, 10, (name) => isc.getRoleByName(name))
                ...
                // Create/update roles
                // ⚡ Bolt: Use processConcurrent with limit 10 instead of unbounded Promise.all
                await processConcurrent(Array.from(roleMap.entries()), 10, async ([roleName, role]) => {
