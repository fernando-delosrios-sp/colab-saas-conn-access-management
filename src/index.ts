...
export const connector = async () => {
    ...
    const stdEntitlementList: StdEntitlementListHandler = async (context, input, res) => {
        ...
        try {
            // ⚡ Bolt: Consolidate entitlement queries across both accessProfiles and roles
            // into a single deduplicated set to avoid redundant API calls for shared queries.
            const allQueries = new Set<string>()
            if (config.accessProfiles) {
                config.accessProfiles.forEach((d) => allQueries.add(d.query))
            }
            if (config.roles) {
                config.roles.forEach((d) => allQueries.add(d.query))
            }

            const globalQueryToEntitlementsMap = new Map<string, EntitlementV2025[]>()
            if (allQueries.size > 0) {
                logger.debug(`Pre-fetching entitlements for ${allQueries.size} unique global queries`)
                const queryResults = await processConcurrent(Array.from(allQueries), async (query) => {
                    const entitlements = await isc.listEntitlements(query)
                    return { query, entitlements }
                })
                queryResults.forEach((res) => globalQueryToEntitlementsMap.set(res.query, res.entitlements))
            }
