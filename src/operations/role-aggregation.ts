...
export async function aggregateRoles(config: Config, isc: ISCClient): Promise<void> {
    ...
    roles: for (const definition of config.roles!) {
        ...
        // ⚡ Bolt: Hoisted loop-invariant deleteStaleRoles condition and ownerId assignment outside the loop, and replaced Map.keys() with Map.entries() to avoid redundant O(1) lookups and improve performance.
        if (deleteStaleRoles) {
