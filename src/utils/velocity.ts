...
function isUnsafeVelocityAST(nodes: any, variables: Map<string, string> = new Map()): boolean {
    ...
    if (typeof nodes === 'object') {
        ...
        if (nodes.type === 'set' && nodes.equal?.length === 2) {
            const [target, expr] = nodes.equal
            if (target?.type === 'references' && target.id) {
                if (expr?.type === 'string') variables.set(target.id, expr.value)
                else if (expr?.type === 'math' && expr.operator === '+' && Array.isArray(expr.expression)) {
                    variables.set(
                        target.id,
                        expr.expression.map((p: any) => (p.type === 'string' ? p.value : '')).join('')
                    )
                }
            }
        }
        ...
        if (nodes.type === 'index' && id) {
            const evalId =
                id.type === 'string' ? id.value : id.type === 'references' && id.id ? variables.get(id.id) : ''
            if (evalId === 'constructor' || evalId === '__proto__' || evalId === 'prototype') return true
        }
