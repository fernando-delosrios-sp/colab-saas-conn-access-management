...
const BANNED_IDENTIFIERS = new Set(['constructor', '__proto__', 'prototype', 'process', 'require', 'global'])
...
function isUnsafeVelocityAST(nodes: any): boolean {
    ...
    if (typeof nodes === 'object') {
        ...
        if (typeof id === 'string' && BANNED_IDENTIFIERS.has(id)) return true

        // Block dynamic or dangerous index accessors
        if (nodes.type === 'index') {
            if (!id || (id.type !== 'string' && id.type !== 'integer')) {
                return true
            }
            if (id.type === 'string' && BANNED_IDENTIFIERS.has(id.value)) {
                return true
            }
        }
...
export function evaluateVelocityExpression(template: string, context: Record<string, unknown> = {}): string {
    ...
    if (!velocity) {
        ...
        if (isUnsafeVelocityAST(velocityTemplate)) {
            throw new Error('Invalid template: execution of banned properties or macros is not allowed')
        }
